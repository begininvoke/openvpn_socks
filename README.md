...
**"Connection reset" or "SIGUSR1\[soft,connection-reset]" right after "VERIFY OK"**

The TLS handshake succeeds but the server closes the connection. Possible causes:

- **Username/password required**: If your provider uses auth in addition to certificates, set `OPENVPN_USERNAME` and `OPENVPN_PASSWORD` (e.g. in a `.env` file next to `docker-compose.yml`).
- **Certificate-only configs**: With no username/password, the reset is often server-side (firewall, geo/IP or connection limits, or strict cipher/options). Try raising verbosity in your `.ovpn` (e.g. `verb 4` or `verb 6`) to see the last message before reset, try another server or port from your provider, or confirm with the provider that your client config and cert are accepted.
